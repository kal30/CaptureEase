...
const PanelDashboard = () => {
  ...
  useEffect(() => {
    ...
    const loadChildren = async () => {
      try {
        ...
        console.log('childrenWithAccess from role context:', childrenWithAccess);
    ...
    const checkParentRole = async () => {
      ...
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          const hasParentRole = userData.roles?.includes('parent') || userData.roles?.includes('primary_parent') || userData.roles?.includes('co_parent') || userData.role === 'parent' || userData.role === 'primary_parent' || userData.role === 'co_parent';
  ...
  useEffect(() => {
    ...
    children.forEach(child => {
      ...
      const quickCompletionRate = Math.round((completedQuickEntries / quickEntries.length) * 100);
      
      // Also factor in detailed entries from database if available
      const childEntries = (recentEntries && recentEntries[child.id]) || [];
      const today = new Date().toDateString();
      const todayEntries = childEntries.filter(entry => 
        new Date(entry.timestamp).toDateString() === today
      );
      
      const expectedDetailedTypes = ['daily_note', 'food_log'];
      const completedDetailedTypes = new Set(todayEntries.map(entry => entry.type));
      const detailedCompletionRate = expectedDetailedTypes.length > 0 
        ? Math.round((completedDetailedTypes.size / expectedDetailedTypes.length) * 100)
        : 0;
      
      // Overall completion is weighted: 70% quick entries + 30% detailed entries
      const overallCompletion = Math.round((quickCompletionRate * 0.7) + (detailedCompletionRate * 0.3));
  ...
  const handleDetailedEntry = (type) => {
    ...
    if (dedicatedPages[actualType]) {
      ...
      console.log('Using tab fallback for:', actualType, 'tab:', tabFallbackMap[actualType]);
      ...
      switch (actualType) {
        ...
        default:
          ...
          alert(`Entry type "${actualType}" will be implemented in the next phase.`);
      }
    }
  ...
  const renderChildCard = (child, groupType) => {
    ...
    const isReadOnly = isReadOnlyForChild ? isReadOnlyForChild(child.id) : false;
    ...
    return (
      <Card 
        ...
        sx={{ 
          border: `2px solid ${completedToday ? theme.palette.success.main : groupStyle.borderColor}`,
          borderRadius: 1,
          overflow: 'hidden',
          bgcolor: completedToday ? alpha(theme.palette.success.main, 0.02) : groupStyle.backgroundColor,
          transition: 'all 0.2s ease',
          cursor: 'pointer',
          '&:hover': {
            boxShadow: completedToday 
              ? `0 4px 20px ${alpha(theme.palette.success.main, 0.2)}`
              : groupStyle.hoverShadow,
      ...
      >
        ...
        <Box sx={{ 
          ...
          <Box sx={{ 
            ...
            <Box sx={{
              ...
              <Typography sx={{ 
                ...
                fontWeight: 600,
                lineHeight: 1,
                pb: 0.25 // Add padding to bottom for better visual centering
              }}>
                {isCategoryExpanded(child.id, 'careTeam') ? '⌃' : '⌄'}
              </Typography>
            </Box>
          </Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                {userRole && (
                  <Chip
                    label={(() => {
                      switch(userRole) {
                        case USER_ROLES.PRIMARY_PARENT: return '👑 Primary Parent';
                        case USER_ROLES.CO_PARENT: return '👨‍👩‍👧‍👦 Co-Parent';
                        case USER_ROLES.FAMILY_MEMBER: return '👵 Family';
                        case USER_ROLES.CAREGIVER: return '🤱 Caregiver';
                        case USER_ROLES.THERAPIST: return '👩‍⚕️ Therapist';
                    ...
                    sx={{
                      height: 20,
                      fontSize: '0.65rem',
                      bgcolor: userRole === USER_ROLES.PRIMARY_PARENT 
                        ...
                        : userRole === USER_ROLES.CO_PARENT
                          ...
                          : userRole === USER_ROLES.THERAPIST
                            ? alpha('#FF6B6B', 0.15)
                            : alpha('#10B981', 0.15),
                      color: userRole === USER_ROLES.PRIMARY_PARENT 
          ...
          <Box sx={{ display: 'flex', gap: 1, px: 2 }}>
            {userRole === USER_ROLES.THERAPIST ? (
              ...
              [
                ...
                { key: 'energy', emoji: '⚡', color: '#FF9800', label: 'Energy' }
              ].map((item) => (
                <Box
                  key={item.key}
                  onClick={(e) => {
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, alignSelf: 'stretch', pt: 1.5 }}>
            {/* Progress Circle */}
              ...
              <Box
                sx={{
                  ...
                  background: `linear-gradient(45deg, ${
                    completedToday ? theme.palette.success.main : theme.palette.primary.main
          ...
          <Box sx={{ p: 2, borderTop: `1px solid ${alpha(theme.palette.divider, 0.2)}` }}>
            ...
            <Box sx={{ mb: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                ...
                <Typography variant="body2" sx={{ fontSize: '0.8rem', fontWeight: 600, color: '#FF6B6B' }}>
                  Diagnosis
                </Typography>
              </Box>
            ...
            <Box sx={{ mb: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                ...
                <Typography variant="body2" sx={{ fontSize: '0.8rem', fontWeight: 600, color: '#9B59B6' }}>
                  Care Team
                </Typography>
              ...
              <Box sx={{ pl: 2 }}>
                  ...
                  <Box sx={{ mb: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                      {child.users.therapists.map((therapistId, index) => (
                        <Chip
                          key={index}
                          label={therapistId === user?.uid ? "You" : `Therapist ${index + 1}`}
                          size="small"
                          sx={{
                            height: 20,
                            fontSize: '0.65rem',
                ...
                {(() => {
                  const members = [
                    ...(child.users?.co_parents || []).map(id => ({ id, role: 'Co-Parent' })),
                    ...(child.users?.family_members || []).map(id => ({ id, role: 'Family' })),
                    ...(child.users?.caregivers || []).map(id => ({ id, role: 'Caregiver' }))
                  ];
                    ...
                    <Box>
                      ...
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                        {members.map((member, index) => {
                          const color = member.role === 'Co-Parent' 
                            ? theme.palette.primary.main
                            : member.role === 'Family' 
                              ? '#EB684A'
                              : '#FF8C42';
                          ...
                          return (
                            <Chip
                              key={index}
                              label={member.id === user?.uid ? `You (${member.role})` : member.role}
                              size="small"
                              sx={{
                                height: 20,
                                fontSize: '0.65rem',
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              {userRole === USER_ROLES.THERAPIST ? (
                ...
                ].filter(buttonGroup => !buttonGroup.hideWhen).map((buttonGroup) => (
                  <Button
                    key={buttonGroup.key}
                    variant="outlined"
                    size="small"
                    startIcon={<span style={{ fontSize: '0.7rem' }}>{buttonGroup.icon}</span>}
                    onClick={(e) => {
                      e.stopPropagation();
                      setCurrentChildId(child.id);
              ...
              {(userRole === 'primary_parent' || userRole === 'co_parent') && (
                <Button
                  variant="outlined"
                  size="small"
                  startIcon={<EditIcon style={{ fontSize: '0.7rem' }} />}
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              {(status.mood || status.sleep || status.energy) && (
                <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', alignItems: 'center', mt: 1 }}>
                  <Typography variant="caption" sx={{ fontSize: '0.65rem', color: 'text.secondary', mr: 1 }}>
                    Logged:
                  </Typography>
                  {status.mood && (
                    <Box sx={{ 
                      ...
                      <Typography variant="caption" sx={{ fontSize: '0.6rem', color: '#E91E63', fontWeight: 500 }}>
                        Mood
                      </Typography>
                    </Box>
                  )}
                  ...
                  {status.sleep && (
                    <Box sx={{ 
                      ...
                      <Typography sx={{ fontSize: '0.6rem' }}>😴</Typography>
                      <Typography variant="caption" sx={{ fontSize: '0.6rem', color: '#673AB7', fontWeight: 500 }}>
                        Sleep
                      </Typography>
                    </Box>
                  )}
                  ...
                  {status.energy && (
                    <Box sx={{ 
                      ...
                      <Typography sx={{ fontSize: '0.6rem' }}>⚡</Typography>
                      <Typography variant="caption" sx={{ fontSize: '0.6rem', color: '#FF9800', fontWeight: 500 }}>
                        Energy
                      </Typography>
                    </Box>
                  )}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="h6" color="text.secondary" sx={{ fontWeight: 400, textAlign: 'left' }}>
                {children.some(child => !isReadOnlyForChild(child.id)) 
                  ? 'Support your child\'s growth with daily tracking and progress insights'
                  : 'Monitor and review children\'s progress and development'
                }
              </Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            ...
            {children?.some(child => {
              ...
              return userRole === USER_ROLES.PRIMARY_PARENT || userRole === USER_ROLES.CO_PARENT;
            }) && (
              <StyledButton
                variant="outlined"
                size="large"
      ...
      <Box sx={{ maxWidth: 800, mx: 'auto' }}>
        {(() => {
          ...
          children?.forEach(child => {
            ...
            } else if (userRole === USER_ROLES.CO_PARENT || userRole === USER_ROLES.FAMILY_MEMBER) {
              familyChildren.push(child);
            } else if (userRole === USER_ROLES.CAREGIVER || userRole === USER_ROLES.THERAPIST) {
