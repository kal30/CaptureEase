...
import IncidentDetails from "../../components/Timeline/parts/IncidentDetails";
import GroupedIncidentDetails from "../../components/Timeline/parts/GroupedIncidentDetails";
import DailyHabitDetails from "../../components/Timeline/parts/DailyHabitDetails";
import JournalDetails from "../../components/Timeline/parts/JournalDetails";
import TherapyNoteDetails from "../../components/Timeline/parts/TherapyNoteDetails";
import EntryHeader from "../../components/Timeline/parts/EntryHeader";
...
const UnifiedTimeline = ({
  ...
  const getEntryPresentation = React.useCallback(
    (entry) => {
      let typeForTimeline;
      if (entry.collection === "incidents") {
        ...
        typeForTimeline =
          getLogTypeByEntry(entry).category ||
          entry.category ||
          entry.timelineType ||
          entry.type ||
          "journal";
  ...
  const getMobileEntryAttachment = React.useCallback((entry, entryType) => {
    ...
    if (
      ...
      return normalizeAttachment(
        entry.mediaURL || entry.mediaUrls?.[0] || entry.mediaAttachments?.[0],
        "image"
      );
    }
        ...
        >
          ...
          {userRole === USER_ROLES.CARE_OWNER && summary.roleBreakdown && (
            <Box sx={{ mt: 1.5, pt: 1.5, borderTop: "1px solid", borderColor: "divider" }}>
              ...
              <Typography variant="body2" color="text.secondary">
                {summary.roleBreakdown.totalContributors} contributor
                {summary.roleBreakdown.totalContributors !== 1 ? "s" : ""}
        ...
        >
          <Stack spacing={1.25} role="list">
            {visibleEntries.map((entry, entryIndex) => {
              ...
              const canEditDesktopEntry =
                entry.collection === "dailyLogs" &&
                ((entry.createdBy || entry.userId || entry.authorId) ===
                  currentUser?.uid);
              const mobileAttachment = getMobileEntryAttachment(
                entry,
                presentation.entryType
              );
              const mobileMeta = getMobileEntryMeta(entry, presentation.entryType);
              const canEditMobileEntry =
                entry.collection === "dailyLogs" &&
                ((entry.createdBy || entry.userId || entry.authorId) ===
                  currentUser?.uid);
